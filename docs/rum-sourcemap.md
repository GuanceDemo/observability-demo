...
产物位于 `dist/observability-demo-rum-sourcemap-2.3.5.zip`，`dist/` 不进入 Git。脚本会同时输出上传所需的 Environment 和 Version；它们必须与 RUM 事件一致。本 Demo 默认 service 为 `mall-h5`，version 来自 `DD_VERSION` 或镜像 tag。
