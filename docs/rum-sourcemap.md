...
RUM 默认关闭。先在可观测平台分别创建商城和 WebGL 游戏两个 Web RUM 应用，取得两个不同且非敏感的 application ID，再设置：
...
helm upgrade --install demo charts/observability-demo \
  ...
  --set-string rum.applicationId=YOUR_STOREFRONT_RUM_APPLICATION_ID \
  --set-string rum.gameApplicationId=YOUR_GAME_RUM_APPLICATION_ID
```

浏览器从 `GET /api/demo/rum-config` 获取开关、商城 `applicationId`、游戏 `gameApplicationId`、project、service、`gameService`、env/version 和同源 `datakitOrigin=/rum-proxy`。商城使用 `applicationId` 与 `service=mall-h5`，ORBITAL DRIFT 使用独立的 `gameApplicationId` 与 `service=mall-game-h5`；分别通过 `RUM_APPLICATION_ID`、`RUM_GAME_APPLICATION_ID`、`RUM_SERVICE`、`RUM_GAME_SERVICE` 或对应 Helm 字段配置。后端兼容直接运行的旧配置：游戏 ID 为空时回退商城 ID；Chart 和 Workshop 在 RUM 开启时要求显式提供两个 ID。RUM、Browser Logs 与请求 baggage 统一使用 Helm 配置的 `project=mall-demo`；响应永远不包含 client token。order-service 将 RUM、Browser Logs 和 Session Replay 请求转发到当前节点 DataKit 的 `9529` 端口，因此不需要 Public DataWay client token。
...
WebGL Replay 验收：切换到“游戏 Demo”，确认手工 View 名为 `game/orbital-drift`，RUM application ID 为独立游戏应用，且上下文包含 `project=mall-demo`、`business_scene=webgl-game`、`preview_mode=web`，service 为 `mall-game-h5`。注入 `game_render_overload` 后无需继续操作游戏；约 10 秒内应看到粒子风暴、HUD FPS 降至约 12、飞船和陨石持续掉帧，但 Replay 仍有连续画面。相同 View 时间轴应包含 `game_render_overload_started` Action、多段约 65ms Long Task、`game_render_overload_recovered` Action，恢复 Action 包含 `actual_fps`、`dropped_frames`、`particle_peak`、`cpu_bursts` 和 `duration_ms`。Guance 与 TrueWatch provider 都应各验证一次，并确认 Replay 继续通过同源 `/rum-proxy` 上传。
