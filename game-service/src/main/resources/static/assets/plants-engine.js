/* Generated by scripts/build-pvz-vendor.py. MIT (c) 2018 Yang Yunhe.
  ...
  setSunTimer () {
    ...
    self.sunTimer = setInterval(function () {
      ...
      let img = document.createElement('img'),                  // 创建元素
          container = document.getElementById('pvzWorld'), // 父级元素容器
          id = self.id,                                         // 当前角色 ID
          top = self.y + 30,
          left = self.x - 130,
      ...
      // 添加阳关元素
      img.src = 'assets/pvz/images/sun.gif'
...
class Game {
  ...
  init () {
    ...
    // 设置键盘按下及松开相关注册函数
    window.PvZLifetime.listen('keydown', function (event) {
      g.keydowns[event.keyCode] = 'down'
    })
    window.PvZLifetime.listen('keyup', function (event) {
...
return {Main, Game, Plant, Zombie, Card, SunNum, Car, Animation, imageFromPath, allImg};
})();
